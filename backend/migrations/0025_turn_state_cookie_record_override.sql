-- 单条探测记录的 Cookie 覆盖：正文存在模型桶上，后续请求直接回放，不再按 pod 取共享池。
alter table account_turn_states add column cookie_override_name text;
alter table account_turn_states add column cookie_override_value text;
alter table account_turn_states add column cookie_override_expires_at bigint;

alter table account_turn_states add constraint account_turn_states_cookie_override_name_length
    check (cookie_override_name is null or char_length(cookie_override_name) <= 64);
alter table account_turn_states add constraint account_turn_states_cookie_override_value_length
    check (cookie_override_value is null or char_length(cookie_override_value) <= 4096);
