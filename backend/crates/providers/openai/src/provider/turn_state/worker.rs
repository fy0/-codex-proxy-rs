...
impl ScheduledTask for TurnStateTask {
    fn run_cycle(&self, context: WorkerCycleContext) -> BoxFuture<'_, Result<(), WorkerTaskError>> {
        Box::pin(async move {
            ...
            futures::stream::iter(buckets.into_iter().filter(|bucket| bucket.config.enabled))
                .for_each_concurrent(4, |bucket| async move {
                    if cancellation.is_cancelled()
                        || bucket.current.as_ref().is_some_and(|token| {
                            token.is_fresh(
                                Utc::now().timestamp(),
                    ...
                    let outcome = tokio::select! {
                        ...
                        outcome = self.service.probe(&bucket, &account_id) => outcome,
                    };
