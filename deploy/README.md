...
探测发往 `chatgpt.com` / `openai.com` 时如果没有可回放 Cookie，会记为 `cookie_required` 并跳过；本机联调地址不强制 Cookie。后续探测和业务请求把账号已保存、且允许回放到该上游的 Cookie 与 state 一起送出。普通 Codex 响应带回的 `__cflb` / `__oai_lb` 会写入账号并在之后回放。已安装的令牌到期后不再注入，旧内容由后台清理，签发水位保留以防旧值重装。
