...
新增轮换时选择 OpenAI OAuth 账号并填写实际上游模型 ID，输入框默认 `gpt-6-astra`，可自定义，配置与注入严格匹配同一个桶。默认关闭自动轮换，普通业务响应仍会采集目标长度候选。
...
3. 在探测记录比较任意头长度的分布；312、552 和其他非目标长度均为 miss，不应产生安装事件。无头与传输错误必须分别出现。
...
长期只有 312 时先核对新身份和出口，再调整预算；单次 miss 不证明实现错误。当前行为是基于实测的显式选用能力。
