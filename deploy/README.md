...
新增轮换时选择 OpenAI OAuth 账号并填写实际上游模型 ID，输入框默认 `gpt-6-astra`，可自定义，配置与注入严格匹配同一个桶。默认关闭自动轮换，普通业务响应仍会采集合法候选。信封合法、在有效期内且签发时间更新的令牌无论长度都保留为候选；目标长度只是安装门槛，长度未命中的候选可以复制，也可在调整目标长度后应用。
...
状态列表有数据时后台刷新不阻断操作。当前票和候选提供按需复制，正文不进入状态轮询和日志；已过期或已替换的票不能复制。详情显示 header 生效来源：模型桶管理的请求优先使用桶内已安装票，缺票时清除覆盖，不回退到账号通用自定义 state；其余模型沿用原有通用覆盖。
...
3. 在探测记录比较任意头长度的分布；312、552 和其他非目标长度均记为长度未命中，合法新票保留为候选但不应产生安装事件，可在候选处复制。无头与传输错误必须分别出现。
...
长期只有 312 时先核对新身份和出口，再将目标长度改为 312；已保留的 312 候选无需重新探测即可直接应用。单次 miss 不证明实现错误。当前行为是基于实测的显式选用能力。
