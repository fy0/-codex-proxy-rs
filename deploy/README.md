...
默认 persona 使用部署 wire profile 的 Core 版本、OS/架构/terminal 信息和 codex-tui UA/originator。
每桶可独立设置探测 originator、User-Agent 与 IANA 时区；UA 留空时按 originator 和部署画像生成。这些配置不改变业务请求的 UA、originator 或出口时区。
时区使用 `Asia/Shanghai`、`Asia/Taipei`、`UTC` 等 IANA 名称。配置弹窗展示完整探测 UA、实际 Core 版本和该时区的日期；自动 UA 跟随运行时已核验的 Core 画像，自定义 UA 不改变独立的 `version` 头。
