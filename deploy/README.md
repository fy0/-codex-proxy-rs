...
顶部表格明确显示是否启用探测改写、当前 state 的获取时间、本次重试数、近期成功获取的平均重试数和预计开始时间。
...
顶部账号显示邮箱与账号 ID；“探测一次”使用桶内配置排队单次尝试，不开启自动轮换。停用轮换的桶保留有效候选，成功记录的“应用此 state”可直接安装，不改变自动探测开关。手动改写同样到期解除；重新保存关闭的轮换配置可提前解除。
被动日志标明请求是否携带自动、手动或客户端 state，并区分 HTTP 响应头、WS 起始及元数据。相同或更旧令牌不会续期；到期后停止覆盖，后续业务恢复无自动 state 请求并继续自然采集。
...
5. 到期后确认 active 变为 false 且后续请求不再注入；凭据过期时应出现跳过记录，不出现探测触发的 refresh 请求。
