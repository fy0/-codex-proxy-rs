...
默认轮换龄为 35 分钟、state 有效期 60 分钟，都从令牌内嵌签发时间开始计龄。升级时旧默认 2700 秒改为 3600 秒，其他自定义值保留。有效期是本地限制，
不是上游 TTL 保证；上游接受窗口变化时应调整配置。已安装的令牌到期后不再注入，旧内容由后台清理，签发水位保留以防旧值重装。
...
4. 若取得有效 292，核对安装签发时间严格增大，再向同一账号、同一上游模型发送业务请求，确认后续请求使用该模型覆盖。其他账号/模型不得使用该值。
