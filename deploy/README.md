...
状态列表有数据时后台刷新不阻断操作。当前票、候选和留存了正文的观测行均提供按需复制，正文不进入状态轮询和日志；已过期的票不能复制，事件正文与槽位正文同寿命、到期摘除。详情显示 header 生效来源：账号通用 state 非空时优先于桶内自动票，改为空后，模型桶管理的请求使用桶内已安装票，缺票时清除覆盖；没有模型桶的请求沿用通用覆盖。
