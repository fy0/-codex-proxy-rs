...
新增轮换时选择 OpenAI OAuth 账号并填写实际上游模型 ID，输入框默认 `gpt-6-astra`，可自定义，配置与注入严格匹配同一个桶。默认关闭自动轮换，普通业务响应仍会采集合法候选。只有命中目标长度的合法新票进入候选；信封合法的其他票正文随观测行留存，可以按签发时间复制，也可在调整目标长度后直接从历史行应用。
