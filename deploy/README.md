...
不是上游保证。当前长度为 292 的 state 大约只在签发后 240 秒内可用，并且后续请求必须同时带上该账号可回放的官方 Cookie，否则不能持续。探测发往 `chatgpt.com` / `openai.com` 时如果没有可回放 Cookie，会记为 `cookie_required` 并跳过；本机联调地址不强制 Cookie。业务请求继续把账号已保存、且允许回放到该上游的 Cookie 与 state 一起送出。已安装的令牌到期后不再注入，旧内容由后台清理，签发水位保留以防旧值重装。
...
4. 若取得有效 292，核对安装签发时间严格增大，再向同一账号、同一上游模型发送业务请求，确认后续请求同时带上该账号可回放的 Cookie，并在大约 240 秒后停止注入。其他账号/模型不得使用该值。
