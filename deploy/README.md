...
状态列表有数据时后台刷新不阻断操作。当前票、候选和留存了正文的观测行均提供按需复制，正文不进入状态轮询和日志；已过期的票不能复制，事件正文与槽位正文同寿命、到期摘除。详情显示 header 生效来源：模型桶管理的请求优先使用桶内已安装票，缺票时清除覆盖，不回退到账号通用自定义 state；其余模型沿用原有通用覆盖。
账号管理的编辑弹窗也按模型显示安装来源、有效期、账号/探测开关与业务状态，提供复制、手动探测、应用候选和移除操作，操作立即生效。模型桶与通用覆盖使用相同的强制应用机制，会话切换后仍覆盖请求头及会话元数据。移除保留签发水位，防止同一旧票重装，不改动账号和探测开关；自动探测开启时会继续寻找更新票，严格策略在缺票期间暂停业务。通用覆盖单独保留，受桶管理的模型不会回退使用它。
