...
`POST /api/admin/accounts/turn-state/copy` 仅供管理员主动复制，接收 `accountId`、实际上游 `model` 和预期 `issuedAt`，返回 `{ "value": "...", "issuedAt": ... }`。只读取身份匹配且尚未过期的已安装票或当前候选；候选不要求命中目标长度，已被替换的历史值不可读取。响应为 `Cache-Control: no-store`；令牌正文不会加入列表、审计或观测日志。
...
| `targetLength` | `292` | 76–4096 字节；安装票的长度门槛。信封合法且在签发水位内更新的令牌无论长度都进入候选池，但只有命中目标长度的候选可以安装 |
...
`candidateIssuedAt` / `candidateLength` 仅返回仍有效候选的签发时间与长度，候选可以是任意观测到的合法长度，不等于可安装。手动应用严格匹配当前候选及账号、模型、上游身份，签发时间必须比已安装值新且长度必须命中 `targetLength`；不会返回或接受令牌正文。`manualOverride=true` 标记手动安装，自动轮换关闭时仍在有效期内注入，但不会启动后台探测；到期停止注入。保存配置保留仍符合目标长度与有效期的已安装票及来源标识；候选只按签发水位与有效期保留，保存配置不因长度不匹配而清除。候选池每桶只保留最新候选，历史日志中的旧值不能重装。
...
`observations` 分别保留最近 100 条主动探测和 100 条被动采集；包含 `source`、`outcome`、`httpStatus`、任意实际头长度 `tokenLength`、可解析的 `issuedAt`、上游在同一响应中声明的 `reportedModel`、脱敏出口 `egress`、`shape`、`effort`、`elapsedMs`、`probeId`、`stopMode` 和 `stopReason`。`reportedModel` 来自上游响应头 `openai-model` / `x-openai-model`、WS 元数据或回应策略窗口内流内事件的模型声明，是上游自报的实际服务模型，与请求模型、桶模型是三个不同字段；缺失为 `null`，不能据此推断请求模型生效，令牌长度本身也不能证明路由。`observedAt` 是获取响应头的时间，`startedAt` 是主动请求开始时间；业务请求无 shape。`transport_error` 与 `missing_header` 分开计数，`invalid_token`、`expired_or_future` 不会安装；`length_miss` 同样不能直接安装，但其合法新票会保留为候选，可在改目标长度后安装或随时复制。回应策略最多读取 1 MiB、30 秒，不保存正文；body 读取超时或错误单独记在 `stopReason`，已收到的合法头仍可安装。安装历史最近 100 条，包含安装时间、签发时间、长度、来源及 `acquiredAt`（头获取时间）、`attempts`（寻获累计尝试数，含成功的一次）、`huntSeconds`。被动获取 attempts 为 0；页面的重试数为 max(attempts−1, 0)，近期平均仅统计这 100 条中的主动成功样本，不能视为所有尝试的平均耗时。无令牌正文、Bearer 或代理认证。
