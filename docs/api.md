...
探测发往 `chatgpt.com` 或 `openai.com` 时，必须同时带上该账号仍有效、且允许回放到该地址的 Cookie。没有可回放 Cookie 时不发请求，结果为 `cookie_required`；Cookie 无法编码成请求头时为 `cookie_invalid`。本机联调地址不强制 Cookie。`__cflb` 和 `__oai_lb` 是账号级路由 Cookie，不绑定某一张 state，也不绑定某一次对话；同一会话可以更换 292，票和 Cookie 错配时上游仍可接受。普通 Codex 响应带回的这两个 Cookie 会写入账号，并在后续探测和业务请求里与 state 一起回放。只有 state、没有仍有效的路由 Cookie 时，上游不会稳定接受这条 292。
