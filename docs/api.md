...
| `POST` | `/api/admin/accounts/turn-state/cookie-apply` | `{ accountId, model, pod }` | 固定共享池中该 pod 当前的 Cookie（按 pod + 签发时间绑定具体值）；服务端事务核对模型和有效期，失败返回 409；Cookie 到期或被续约替换后固定自动失效 |
