...
| `ttlSeconds` | `240` | 60–3600 秒，从令牌内嵌签发时间计算的本地有效期；重复观察不续期。当前上游对 292 的接受窗口大约为 240 秒 |
...
探测发往 `chatgpt.com` 或 `openai.com` 时，必须同时带上该账号仍有效、且允许回放到该地址的 Cookie。没有可回放 Cookie 时不发请求，结果为 `cookie_required`；Cookie 无法编码成请求头时为 `cookie_invalid`。本机联调地址不强制 Cookie。业务请求继续把账号已保存的 Cookie 与 state 一起送出；只有 state、没有对应 Cookie 时，上游不会持续接受这条 292。
