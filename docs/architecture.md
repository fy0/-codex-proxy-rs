...
解析器解释，Admin 按中立的额度事实选择近期进度段。该采样不改变全站完整交付用量口径，不创建第二份
...
Turn-state 由 OpenAI Provider 收集响应头并贡献 `openai-turn-state` 定时任务，Host 沿用 Redis lease 和统一取消/监督机制运行，不引入外部轮换进程。Core 定义账号/模型桶及令牌信封的共享值合同；Store 持久化候选、模型专属覆盖和有界观测，使用同桶事务和签发时间严格递增条件原子安装。Admin 通过账号端口配置，API 只处理管理 wire，常规状态查询不读取令牌正文，管理员复制走独立按需入口。自动令牌绝不提升成账号级通用覆盖。重新授权改变上游账号或用户身份时，同一事务中的数据库触发器清除旧令牌与历史、保留配置；在途观测与发送前注入均核对桶的上游身份，避免本地账号 ID 复用导致串号。安装、移除与配置变更使该账号旧 WS 连接失效，后续请求按准确的账号与模型读取覆盖。探测读取现有 OAuth 凭据，绝不调用 refresh，并且不携带账号 Cookie。`__oailb`（兼容 `__oai_lb`）路由 Cookie 按端点和网关 pod 共享保存，模型桶可按允许的 `unified-*` 编号自动选择，也可事务固定某一候选；它和 state 分开保存，不和某一张票绑定，只随业务请求回放。实际模型检测开启且桶为暂停业务调度时，已安装票上的实际模型突然变化会作废该票并保留签发水位。请求直接走 Provider 上游客户端，不进入 Core 业务执行与计量路径。过期或未来签发的令牌不会成为可用候选，寿命从内嵌签发时间计算，默认 240 秒。

手动探测由 Admin 在同一模型桶写入单槽请求，重复排队合并；Provider worker 原子领取后执行一次，绕过自动轮换的计龄与等待条件，不改变自动轮换开关。领取后取消或进程退出不会重放该请求。探测 UA、originator 和时区属于桶配置，不改变业务请求画像；被动观测记录请求 state 来源，相同或更旧的返回值不续期、不安装。

Admin 的手动应用与 Provider 自动安装复用 Store 的同桶原子更新门；手动应用携带预期签发时间，拒绝已经被替换的候选。手动标识记录安装来源，不授权后台持续探测。手动安装、审计和配置版本同事务提交；自动安装提交桶的运行时事实。业务请求仍核对上游身份及有效期，WS 按含 state 的连接画像隔离。

Admin 移除已安装票时同样按桶和预期签发时间比较更新，与审计、版本同事务提交并发布快照。移除不改变账号或探测开关，保留历史签发水位防止在途响应重装旧票；更新候选仍可正常安装。`hasInstalledState` 区分有效正文与仅存水位，不与手动停用混淆。模型桶与账号通用覆盖复用强制注入函数，在会话作用域处理后统一改写 header 和 metadata；观测来源区分模型桶自动安装、手动应用与账号通用覆盖，不把发送来源当作响应安装结果。

可选飞书通知沿用安装事务，在 Store 保存每桶最新安装的通知任务与重试进度，不复制令牌正文。Provider 的轮换 worker 并行处理探测与有界通知发送，Admin 不直接调用机器人。任务原子领取时核对当前票、有效期、上游身份及账号开关，确认按签发时间比较更新，避免旧发送结果确认新票。通知使用 Core 的数据库中立端口；Webhook HTTP 合同由 OpenAI turn-state 模块拥有，不进入业务执行链路。进程崩溃允许重试，远端送达与本地确认之间的窗口可能重复；记录失败原因时不输出 Webhook、state 或远端响应正文。

无票调度策略属于账号/实际上游模型桶，默认允许；严格模式只限制业务，绝不修改手动账号开关或自动探测开关。
Core 的桶合同统一判定已安装票的身份、目标长度与内嵌签发有效期；OpenAI 选号按映射后模型读取现有 Store 端口，
将有票有效期投影为 `AccountRuntimeSignals`，由统一资格判断拒绝 `MissingTurnState`，不计入可等待容量。
粘性、continuation、重试与复用连接的新请求共用选号；选中的桶与租约一起传递至发送前注入，冷流启动时再次核对期限，
已经发出的流没有票据到期中断。安装提交后新选号直接读到新票，到期判断不依赖清理任务或缓存 TTL。
配置仍沿用审计、revision 与快照发布链路，已有 WS 画像隔离和失效机制不变；候选、失败探测与账号级通用覆盖均不能解除缺票限制。
关闭自动探测或保存策略保留仍有效的已安装票，手动探测与应用是独立恢复入口。

管理员主动复制通过独立的按需读取端口取得仍有效的当前票或候选，Store 在读取时核对桶身份与签发时间；常规列表、审计和观测不返回正文，复制响应不可缓存。停用账号的观测不落库，关闭自动探测只停止后台观测日志，不丢弃有效被动候选；显式手动探测保留结果。UA 预览和发送共用 Provider 的生成逻辑；探测 CLI 版本由桶配置拥有，只接受正式版本，自动 UA 与 version 头保持一致，部署画像仅提供环境字段，不将 Desktop 内嵌 Core 的预发布版本用于 CLI 探测。
