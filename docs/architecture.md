...
Admin 的手动应用与 Provider 自动安装复用 Store 的同桶原子更新门；手动应用携带预期签发时间，拒绝已经被替换的候选。手动标识允许关闭自动轮换时注入当前有效令牌，不授权后台持续探测。安装、审计和配置版本同事务提交，业务请求仍核对上游身份及有效期，WS 按含 state 的连接画像隔离。
