...
管理员主动复制通过独立的按需读取端口取得仍有效的当前票或候选，Store 在读取时核对桶身份与签发时间；常规列表、审计和观测不返回正文，复制响应不可缓存。停用账号的观测不落库，关闭自动探测只停止后台观测日志，不丢弃有效被动候选；显式手动探测保留结果。UA 预览和发送共用 Provider 运行时画像的生成逻辑。
